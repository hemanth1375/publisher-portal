...
export class ViewapiPageService {
  ...
  addEndPoint(id:any,obj:any){
    const url=urls.addEndpoint+`?krakendId=${id}`
    const headers={
  ...
  updateEndPoint(id:any,obj:any){
    const url=urls.updateEndpoint+`?endpointId=${id}`
    const headers={
  ...
  updatebackend(id:any,obj:any){
    const url=urls.updateBackend+`?backendId=${id}`
    return this.http.post(url,obj)
  }
}
